...
int_seq <- "no_superspreading_event"
...
ic_real <- lapply(1:length(pred_period), function(i) ic_all %>% filter(Date %in%pred_period[[i]]) %>% group_by(sim_num) %>%
                    summarise(OnCampus=sum(OnCampus),OffCampus=sum(OffCampus) ) )

# predicted diagnosed counts
ic_pred <- lapply(1:length(pred_period), function(i) ic_all %>% filter(Date %in%( pred_period[[i]] -length(pred_period[[i]]) ) )%>% group_by(sim_num) %>% summarise(OnCampus=sum(OnCampus),OffCampus=sum(OffCampus) ) )


# estimate ratios
quar_count <- lapply(1:length(pred_period), function(i)  quar_index %>% select(-c(q_off_ind_on_off,q_on_ind_on_off))%>% filter(Date %in%ratio_period[[i]])%>% group_by(sim_num) %>%  summarise(q_off_ind_off=sum(q_off_ind_off),q_off_ind_on=sum(q_off_ind_on),q_on_ind_off=sum(q_on_ind_off),q_on_ind_on=sum(q_on_ind_on)) )
...
# predicted quarantine counts
quar_pred <- sapply(1:length(pred_period), function(i) merge(ratio_est[[i]],ic_pred[[i]]) %>% mutate(count=(ratio1*OnCampus+ratio2*OffCampus) ) %>% 
